...
import com.lovisgod.kozenlib.core.utilities.DateUtils
import org.simpleframework.xml.Element
import java.util.*

sealed class TransactionRequestKozen {
    @field:Element(name = "terminalInformation", required = false)
    var terminalInformation: TerminalInformationRequestKozen? = null

    @field:Element(name = "cardDataKozen", required = false)
    var cardDataKozen: CardDataKozen? = null
    ...
    @field:Element(name = "pinDataKozen", required = false)
    var pinDataKozen: PinDataKozen? = null
    ...
    companion object {
        ...
        fun createPurchaseRequest(terminalInfoX: TerminalInfo, requestData: RequestIccData = RequestIccData()): TransactionRequestKozen {
            val request = PurchaseRequestKozenKozen()
            var cardDataKozenX = CardDataKozen.create(requestData)

            return request.apply {
                terminalInformation =
                    TerminalInformationRequestKozen().createForrequest(
                        terminalInfo = terminalInfoX, haspin = requestData.haspin!!)
                cardDataKozen = cardDataKozenX
                minorAmount = requestData.TRANSACTION_AMOUNT
                stan = Constants.getNextStan()
                pinDataKozen = if (requestData.haspin!!) PinDataKozen.create(Constants.memoryPinData) else null
        ...
        fun createCashoutRequest(terminalInfoX: TerminalInfo, requestData: RequestIccData = RequestIccData()): TransactionRequestKozen {
            val request = TransferRequestKozenKozen()
            var cardDataKozenX = CardDataKozen.create(requestData)
